                ...
                <form onSubmit={handleSubmit} className='flex h-full flex-col justify-center gap-4 overflow-y-auto p-5 sm:p-8'>
                    <div className='space-y-1.5'>
                        <div className='flex items-center gap-3'>
                            <img src="/favicon.svg" alt="QuickChat" className='h-9 w-9 rounded-full' />
                            <p className='text-xs font-medium text-[#00a884]'>Profile</p>
                        </div>
                        <h3 className='text-[28px] font-light'>Edit profile</h3>
                        <p className='text-xs text-[#667781]'>Update your photo, name and about.</p>
                    </div>
                    <label htmlFor="avatar" className='flex cursor-pointer items-center gap-3 rounded-[10px] border border-[#dfe5e7] bg-white p-3'>
                        <input onChange={(e) => setSelectedImage(e.target.files[0])} type="file" id='avatar' accept='image/*' hidden />
                        <img src={selectedImage ? URL.createObjectURL(selectedImage) : authUser?.profilePic || assets.avatar_icon} alt="" className='h-14 w-14 rounded-full object-cover'/>
                    ...
                    <div className='flex gap-3'>
                        ...
                        <button type='button' onClick={() => navigate('/')} className='rounded-[10px] border border-[#d1d7db] px-5 py-2.5 text-sm text-[#667781] transition hover:border-[#00a884] hover:text-[#111b21]'>Cancel</button>
                    </div>
                </form>
