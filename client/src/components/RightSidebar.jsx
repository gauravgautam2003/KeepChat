    ...
    return selectedUser && (
        <div className='hidden min-h-0 border-l border-[#d1d7db] bg-[#f0f2f5] text-[#111b21] lg:flex lg:flex-col'>
            ...
            <div className='flex-1 min-h-0 overflow-y-auto p-6'>
                <div className='flex flex-col items-center gap-3 rounded-[10px] bg-white p-6 text-center shadow-[0_1px_3px_rgba(11,20,26,0.08)]'>
                    ...
                    <p className='text-sm leading-6 text-[#667781]'>{selectedUser.bio || "No status added yet."}</p>
                </div>

                <div className='mt-6 rounded-[10px] bg-white p-5 shadow-[0_1px_3px_rgba(11,20,26,0.08)]'>
                    <p className='text-sm font-medium text-[#41525d]'>Shared media</p>
                    <div className='mt-4 grid grid-cols-2 gap-3'>
                        {messageImages.length ? messageImages.map((url,index)=>(
                            ...
                            <p className='col-span-2 text-sm text-[#667781]'>No images shared in this chat yet.</p>
                        )}
                    </div>
                </div>
                <button onClick={() => logout()} className='mt-6 w-full rounded-[10px] bg-white px-4 py-3 text-sm font-medium text-[#c65353] shadow-[0_1px_3px_rgba(11,20,26,0.08)] transition hover:bg-[#fff5f5]'>Logout</button>
            </div>
        </div>
    )
}
