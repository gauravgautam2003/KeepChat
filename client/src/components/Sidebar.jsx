...
const Sidebar = () => {
    ...
    const { getUsers, users = [], selectedUser, setSelectedUser, unseenMessages = {}, setUnseenMessages, isPreviewMode } = useContext(ChatContext) || {};
    ...
    return (
        <div className={`min-h-0 border-r border-[#d1d7db] bg-[#ffffff] text-[#111b21] ${selectedUser ? "hidden lg:block" : ""}`}>
            <div className='flex h-full min-h-0 flex-col'>
                    ...
                    <div className='flex items-center justify-between'>
                        <div className='flex items-center gap-3'>
                            ...
                            <div>
                                {authUser ? (
                                    <>
                                        <p className='text-sm font-semibold text-[#111b21]'>{authUser?.name}</p>
                                        <p className='max-w-[192px] text-xs text-[#667781] shrink-0 truncate'>{authUser?.bio}</p>
                                    </>
                                ) : (
                                    <>
                                        <p className='text-sm font-semibold text-[#111b21]'>Visitor Preview</p>
                                        <p className='max-w-[192px] text-xs text-[#667781] shrink-0 truncate'>Explore the app before signing up</p>
                                    </>
                                )}
                            </div>
                        </div>
                        <div ref={menuRef} className='relative'>
                            ...
                            <div className={`absolute right-0 top-full z-20 mt-2 w-44 rounded-md border border-[#d1d7db] bg-white p-2 shadow-xl ${isMenuOpen ? 'block' : 'hidden'}`}>
                                {authUser ? (
                                    ...
                                    <button onClick={() => { setIsMenuOpen(false); navigate("/login") }} className='flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm text-[#111b21] transition hover:bg-[#f5f6f6]'>
                                        <FaUserPlus />
                                        Create account
                                    </button>
                                )}
                ...
                {isPreviewMode && (
                    <div className='border-b border-[#d9efe8] bg-[#ecfff7] px-4 py-3 text-xs text-[#0b5f4f]'>
                        Preview mode is read-only. Open chats freely, but messaging starts after account creation.
                    </div>
                )}
                            ...
                            <button onClick={() => { setSelectedUser(user); setUnseenMessages((prev) => ({ ...prev, [user._id]: 0 })) }} key={user._id} className={`relative flex min-w-0 items-center gap-3 px-4 py-3 text-left transition hover:bg-[#f5f6f6] ${selectedUser?._id === user._id ? 'bg-[#f0f2f5]' : ''}`}>
                                ...
                                <div className='min-w-0 flex-1 overflow-hidden border-b border-[#f0f2f5] pb-3 pr-10'>
                                    <div className='flex items-center justify-between gap-3'>
                                        ...
                                        <span className={`max-w-[92px] shrink-0 absolute right-2 truncate text-[11px] ${(authUser ? onlineUsers.includes(user._id) : user.isPreviewOnline) ? 'text-[#00a884]' : 'text-[#667781]'}`}>{(authUser ? onlineUsers.includes(user._id) : user.isPreviewOnline) ? 'Active now' : formatLastSeen(user.lastSeen)}</span>
                                    </div>
